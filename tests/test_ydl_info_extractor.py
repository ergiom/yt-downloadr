...
class TestYdlInfoExtractor:
    def test_extract_info_raises_when_ydl_raises(self):
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'extract_info', MockYdl.extract_info_error)
    ...
    def test_sanitize_info_raises_when_ydl_raises(self):
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'extract_info', MockYdl.extract_info_correct)
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'sanitize_info', MockYdl.sanitize_info_error)
        ydl_extractor = YdlInfoExtractor()
        with pytest.raises(InfoExtractorError):
            ydl_extractor.extract('some_link')
    ...
    def test_raises_when_received_incomplete_info(self):
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'extract_info', MockYdl.extract_info_correct)
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'sanitize_info', MockYdl.sanitize_info_incomplete_no_formats)
        ydl_extractor = YdlInfoExtractor()
        with pytest.raises(InfoExtractorError):
            ydl_extractor.extract('some_link')
    ...
    def test_passes_when_some_formats_incomplete(self):
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'extract_info', MockYdl.extract_info_correct)
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'sanitize_info', MockYdl.sanitize_info_correct_one_format_complete_one_format_incomplete)
        ydl_extractor = YdlInfoExtractor()
    ...
    def test_passes_when_correct(self):
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'extract_info', MockYdl.extract_info_correct)
        pytest.MonkeyPatch().setattr(yt_dlp.YoutubeDL, 'sanitize_info', MockYdl.sanitize_info_correct)
        ydl_extractor = YdlInfoExtractor()
        info = ydl_extractor.extract('some_link')
        assert info == Returns.returned_basic_info()
